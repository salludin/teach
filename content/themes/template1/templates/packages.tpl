                            ...
                            <ul class="list-group list-group-flush text-center">
                                ...
                                <li class="list-group-item">
                                    {if !$package['boost_posts_enabled']}
                                        <i class="fa fa-times red mr10"></i>{__("Job Posting")}
                                    {else}
                                        <i class="fa fa-check green mr10"></i>{__("Job Posting")} {$package['boost_posts']}
                                    {/if}
                                </li>
                                <li class="list-group-item">
                                    {if !$package['boost_pages_enabled']}
                                        <i class="fa fa-times red mr10"></i>{__("Talent Search")}
                                    {else}
                ...
                {foreach $packages as $package}
                    ...
                    <div class="col-md-6 col-lg-4 col-xl-{if $packages_count >= 4}3{elseif $packages_count == 3}4{elseif $packages_count <= 2}6{/if} text-center">
                        <div class="card card-pricing shadow-sm">
                            ...
                            <div class="card-body text-center">
                                <h3 class="price" style="color: {$package['color']}">{$system['system_currency_symbol']}{$package['price']}</h2>
                                <div>
                                    {if $package['period'] == "life"}
                                        {__("Life Time")}
                                    {else}
                            ...
                            <ul class="list-group list-group-flush text-center">
                                <li class="list-group-item"><i class="fa fa-check green mr10"></i>{__("300x250 mm ad size on the school website page and user website page of Teachin.id.")}</li>
                                <li class="list-group-item"><i class="fa fa-check green mr10"></i>{__("Ads on Mobile Apps and Teachin Social Media")}</li>
                            </ul>
                            <div class="card-footer bg-transparent">
                                {if $user->_logged_in}
                                    <button class="btn btn-block rounded-pill btn-danger" data-toggle="modal" data-url="#payment" data-options='{literal}{{/literal}"handle": "packages", "id": {$package["package_id"]}, "price": "{$package["price"]}", "name": "{$package["name"]}", "img": "{$package["icon"]}"{literal}}{/literal}'>
                                ...
                                {else}
                                    <a class="btn btn-block rounded-pill btn-danger" href="{$system['system_url']}/upads">
                                        {__("Buy Now")}
                                    </a>
                                {/if}
                            </div>
