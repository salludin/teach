...
<!-- post text -->
{if $_post['post_type'] != "product" && $_post['post_type'] != "offer" && $_post['post_type'] != "job"}
    {if !$_shared}
...
{elseif $_post['post_type'] == "job" && $_post['job']}
    <div class="mt10">
        <div class="post-media">
            <a class="post-media-image" href="{$system['system_url']}/posts/{$_post['post_id']}" target="_blank">
                <div class="image" style="background-image:url('{$_post['job']['cover_image']}');padding-top:100%"></div>
                {if $_post['job']['hide_salary'] == '1'}
                {else}
                <div class="source">
                    <strong>{$system['system_currency_symbol']}{$_post['job']['salary_minimum']} - {$system['system_currency_symbol']}{$_post['job']['salary_maximum']} / {$_post['job']['pay_salary_per_meta']}</strong>
                </div>
                {/if}
                <div class="icon">
        ...
        <div class="post-job-meta">
            ...
            {/if}
            {if $_post['job']['available'] &&  $_post['author_id'] != $user->_data['user_id'] }
                <div class="mt10 clearfix">
