...
class User {
        ...
        if($get_notifications->num_rows > 0) {
            while($notification = $get_notifications->fetch_assoc()) {
                ...
                switch ($notification['action']) {
                    ...
                    case 'live_stream':
                        $notification['icon'] = "fa fa-signal";
                        $notification['url'] = $system['system_url'].'/posts/'.$notification['node_url'];
    ...
    public function post_notification($args = []) {
        ...
        if($system['email_notifications']) {
            ...
            if($notification['message']) {
                ...
                /* send email */
                _email($receiver['user_email'], $subject, $body['html']);
            }
        }
     ...
     * @return integer
     */
    public function create_live_post($agora_uid, $agora_token, $agora_channel_name, $video_thumbnail) {
        global $db, $system, $date;
    ...
    public function start_live_recording($post_id, $agora_uid, $agora_token, $agora_channel_name) {
        ...
        curl_setopt($ch, CURLOPT_HTTPHEADER, array('Authorization: Basic '.$base64Credentials,'Content-Type: application/json;charset=utf-8'));
    ...
    public function get_live_post_stats($post_id, $last_comment_id) {
        ...
        /* total live users */
        $get_live_users = $db->query(sprintf("SELECT COUNT(*) as count FROM posts_live_users WHERE post_id = %s", secure($post_id, 'int') )) or _error("SQL_ERROR");
     ...
     * @return string
     */
    public function agora_token_builder($is_host = true, $channel_name = null) {
        global $system;
        require_once(ABSPATH.'includes/libs/Agora/RtcTokenBuilder.php');
    ...
    public function get_posts($args = []) {
        ...
        if(!in_array($filter, array('all', '', 'link', 'media', 'live', 'photos', 'map', 'product', 'offer', 'job', 'article', 'poll', 'video', 'audio', 'file'))) {
            _error(400);
        }
            ...
            case 'offer':
                ...
                switch ($post['offer']['discount_type']) {
                    ...
                    case 'discount_amount':
                        $post['offer']['meta_title'] = $system['system_currency_symbol'].$post['offer']['discount_amount']." ".__("Off")." ".$post['offer']['title'];
                        break;
                    ...
                    case 'spend_get_off':
                        $post['offer']['meta_title'] = __("Spend")." ".$system['system_currency_symbol'].$post['offer']['spend_x']." ".__("Get")." ".$system['system_currency_symbol'].$post['offer']['amount_y']." ".__("Off")." ".$post['offer']['title'];
                        break;
    ...
    public function edit_product($post_id, $message, $args = []) {
        ...
        $db->query(sprintf("UPDATE posts_products SET name = %s, price = %s, category_id = %s, status = %s, location = %s WHERE post_id = %s", secure($args['name']), secure($args['price']), secure($args['category_id'], 'int'), secure($args['status']), secure($args['location']), secure($post_id, 'int') )) or _error("SQL_ERROR_THROWEN");
    }
    ...
    public function edit_job($post_id, $message, $args = []) {
        ...
        $db->query(sprintf("UPDATE posts SET text = %s WHERE post_id = %s", secure($message), secure($post_id, 'int') )) or _error("SQL_ERROR_THROWEN");
        /* update job */
        $db->query(sprintf("UPDATE posts_jobs SET 
            category_id = %s, 
            title = %s, 
            location = %s, 
            salary_minimum = %s, 
    ...
    /**
     * get_offers_categories_ids
     * 
     * @return array
     */
    public function get_offers_categories_ids() {
        global $db;
        $categories = [];
        $get_categories = $db->query("SELECT category_id FROM offers_categories") or _error("SQL_ERROR_THROWEN");
        if($get_categories->num_rows > 0) {
            while($category = $get_categories->fetch_assoc()) {
                $categories[] = $category['category_id'];
            }
     ...
     * @return array
     */
    public function get_offers_categories($sorted = true) {
        global $db;
        $categories = [];
        $order_query = ($sorted)? " ORDER BY category_order ASC " : " ORDER BY category_id ASC ";
        $get_categories = $db->query("SELECT * FROM offers_categories".$order_query) or _error("SQL_ERROR_THROWEN");
        if($get_categories->num_rows > 0) {
            while($category = $get_categories->fetch_assoc()) {
                $category['category_url'] = get_url_text($category['category_name']);
                $categories[] = $category;
    ...
    /**
     * get_jobs_categories_ids
     * 
     * @return array
     */
    public function get_jobs_categories_ids() {
        global $db;
        $categories = [];
        $get_categories = $db->query("SELECT category_id FROM jobs_categories") or _error("SQL_ERROR_THROWEN");
        if($get_categories->num_rows > 0) {
            while($category = $get_categories->fetch_assoc()) {
                $categories[] = $category['category_id'];
            }
     ...
     * @return array
     */
    public function get_jobs_categories($sorted = true) {
        global $db;
        $categories = [];
        $order_query = ($sorted)? " ORDER BY category_order ASC " : " ORDER BY category_id ASC ";
        $get_categories = $db->query("SELECT * FROM jobs_categories".$order_query) or _error("SQL_ERROR_THROWEN");
        if($get_categories->num_rows > 0) {
            while($category = $get_categories->fetch_assoc()) {
                $category['category_url'] = get_url_text($category['category_name']);
                $categories[] = $category;
    ...
    public function get_games($offset = 0, $played = false) {
        ...
        if($played) {
            ...
            $get_games = $db->query(sprintf('SELECT * FROM games ORDER BY game_id ASC LIMIT %s, %s', secure($offset, 'int', false), secure($system['max_results_even'], 'int', false) )) or _error("SQL_ERROR_THROWEN");
        }
    ...
    public function ads_campaigns($place = 'sidebar') {
        ...
        $get_campaign_analyst = $db->query(sprintf("SELECT ads_campaigns.*, ads_campaigns_analyst.campaign_id FROM ads_campaigns INNER JOIN ads_campaigns_analyst ON ads_campaigns.campaign_id = ads_campaigns_analyst.campaign_id WHERE ads_campaigns.campaign_id = %s AND date = DATE(NOW())", secure($campaign['campaign_id'], 'int') )) or _error("SQL_ERROR_THROWEN");
        if($get_campaign_analyst->num_rows == 0) {
            if($this->_data['user_gender'] == 'male') {
            $db->query(sprintf("INSERT INTO ads_campaigns_analyst (campaign_id, views, male, date) VALUES (%s, 1, 1, %s)", secure($campaign['campaign_id'], 'int'), secure($date) )) or _error("SQL_ERROR_THROWEN");    
            }elseif ($this->_data['user_gender'] == 'female'){
            $db->query(sprintf("INSERT INTO ads_campaigns_analyst (campaign_id, views, female, date) VALUES (%s, 1, 1, %s)", secure($campaign['campaign_id'], 'int'), secure($date) )) or _error("SQL_ERROR_THROWEN"); 
            }
           $db->query(sprintf("INSERT INTO ads_campaigns_analyst (campaign_id, views, date) VALUES (%s, 1, %s)", secure($campaign['campaign_id'], 'int'), secure($date) )) or _error("SQL_ERROR_THROWEN"); 
        }else{
            if($this->_data['user_gender'] == 'male') {
            $db->query(sprintf("UPDATE ads_campaigns_analyst SET views = views + 1, male = male + 1 WHERE campaign_id = %s AND date = DATE(NOW())", secure($campaign['campaign_id'], 'int') )) or _error("SQL_ERROR_THROWEN");    
            }elseif ($this->_data['user_gender'] == 'female'){
            $db->query(sprintf("UPDATE ads_campaigns_analyst SET views = views + 1, female = female + 1 WHERE campaign_id = %s AND date = DATE(NOW())", secure($campaign['campaign_id'], 'int') )) or _error("SQL_ERROR_THROWEN"); 
            }
           $db->query(sprintf("UPDATE ads_campaigns_analyst SET views = views + 1 WHERE campaign_id = %s AND date = DATE(NOW())", secure($campaign['campaign_id'], 'int') )) or _error("SQL_ERROR_THROWEN"); 
        }
                // update campaign
                /* [1] -> campaign expired */
    ...
    /**
     * get_campaign_analyst
     * 
     * @param integer $campaign_id
     * @return array
     */
    public function get_campaign_analyst($campaign_id) {
        global $db;
        $get_campaign = $db->query(sprintf("SELECT ads_campaigns_analyst.*, ads_campaigns.campaign_id,ads_campaigns.campaign_title FROM ads_campaigns_analyst INNER JOIN ads_campaigns ON ads_campaigns_analyst.campaign_id = ads_campaigns.campaign_id WHERE ads_campaigns_analyst.campaign_id = %s", secure($campaign_id, 'int') )) or _error("SQL_ERROR_THROWEN");
        if($get_campaign->num_rows == 0) {
            return false;
        }
        if($get_campaign->num_rows > 0) {
            while($campaign = $get_campaign->fetch_assoc()) {
                $campaigns[] = $campaign;
            }
        }
        return $campaigns;
    ...
    public function update_campaign_bidding($campaign_id) {
        ...
        $get_campaign_analyst = $db->query(sprintf("SELECT ads_campaigns.*, ads_campaigns_analyst.campaign_id FROM ads_campaigns INNER JOIN ads_campaigns_analyst ON ads_campaigns.campaign_id = ads_campaigns_analyst.campaign_id WHERE ads_campaigns.campaign_id = %s AND date = DATE(NOW())", secure($campaign_id, 'int') )) or _error("SQL_ERROR_THROWEN");
        if($get_campaign_analyst->num_rows == 0) {
           $db->query(sprintf("INSERT INTO ads_campaigns_analyst (campaign_id, views, clicks, date) VALUES (%s, 1, 1, %s)", secure($campaign_id, 'int'), secure($date) )) or _error("SQL_ERROR_THROWEN"); 
        }else{
           $db->query(sprintf("UPDATE ads_campaigns_analyst SET clicks = clicks + 1 WHERE campaign_id = %s AND date = DATE(NOW())", secure($campaign_id, 'int') )) or _error("SQL_ERROR_THROWEN"); 
        }
        $campaign = $get_campaign->fetch_assoc();
    ...
    public function send_invitation_code($code, $email) {
        ...
        $body = get_email_template("invitation_system_email", $subject, ["code" => $code]);
        /* send email */
        if(!_email($email, $subject, $body['html'])) {
    ...
    public function send_invitation_email($email) {
        ...
        $body = get_email_template("invitation_user_email", $subject);
        /* send email */
        if(!_email($email, $subject, $body['html'])) {
            throw new Exception(__("Invitation email could not be sent"));
        }
    ...
    public function sign_up_company($args = []) {
        global $db, $system, $date;
        /* check invitation code */
        if(!$system['registration_enabled'] && $system['invitation_enabled']) {
            if(!$this->check_invitation_code($args['invitation_code'])) {
        ...
        /* check IP */
        $this->_check_ip();
        if(is_empty($args['company_name']) || is_empty($args['username']) || is_empty($args['password'])) {
            throw new Exception(__("You must fill in all of the fields"));
        }
        if(!valid_username($args['username'])) {
            throw new Exception(__("Please enter a valid username (a-z0-9_.) with minimum 3 characters long"));
        }
        if(reserved_username($args['company_name'])) {
            throw new Exception(__("You can't use")." <strong>".$args['username']."</strong> ".__("as username"));
        }
        if($this->check_username($args['company_name'])) {
            throw new Exception(__("Sorry, it looks like")." <strong>".$args['username']."</strong> ".__("belongs to an existing account"));
        }
        if(!valid_email($args['email'])) {
            throw new Exception(__("Please enter a valid email address"));
        ...
        if(!valid_name($args['company_name'])) {
            throw new Exception(__("Your Institution name contains invalid characters"));
        }
        if(strlen($args['company_name']) < 3) {
            throw new Exception(__("Your Institution name must be at least 3 characters long. Please try another"));
        }
        /* set custom fields */
        $custom_fields = $this->set_custom_fields($args);
        /* check reCAPTCHA */
        ...
        $phone_verification_code = ($system['activation_enabled'] && $system['activation_type'] == "sms")? get_hash_key(6, true) : 'null';
        /* register user */
        $db->query(sprintf("INSERT INTO users (user_group, user_name, user_email,  user_password, user_activated, user_firstname, user_gender, user_registered, user_email_verification_code, user_phone, user_phone_verification_code, user_privacy_newsletter) VALUES ('4', %s, %s, %s, '1', %s, 'other', %s, %s, %s, %s, %s)", secure($args['username']), secure($args['email']), secure(_password_hash($args['password'])), secure(ucwords($args['first_name'])), secure($date), secure($email_verification_code), secure($args['phone']), secure($phone_verification_code), secure($newsletter_agree) )) or _error("SQL_ERROR_THROWEN");
        /* get user_id */
        $user_id = $db->insert_id;
        $db->query(sprintf("INSERT INTO pages (page_admin, page_category, page_name, page_title, page_description, page_date) VALUES (%s, %s, %s, %s, '', %s)", secure($user_id), secure($args['category'], 'int'), secure($args['username']), secure($args['company_name']), secure($date) )) or _error("SQL_ERROR_THROWEN");
        /* get page_id */
        $page_id = $db->insert_id;
        $db->query(sprintf("INSERT INTO pages_admins (user_id, page_id) VALUES (%s, %s)", secure($user_id, 'int'),  secure($page_id, 'int') )) or _error("SQL_ERROR_THROWEN");
        /* insert custom fields values */
        if($custom_fields) {
            foreach($custom_fields as $field_id => $value) {
                $db->query(sprintf("INSERT INTO custom_fields_values (value, field_id, node_id, node_type) VALUES (%s, %s, %s, 'user')", secure($value), secure($field_id, 'int'), secure($user_id, 'int') ));
        ...
        if($user['user_two_factor_enabled']) {
            ...
            switch ($system['two_factor_type']) {
                case 'email':
                    ...
                    /* send email */
                    if(!_email($user['user_email'], $subject, $body['html'])) {
    ...
    public function payment_information($email, $event_name, $url) {
        global $db, $system;
        $subject = 'Informasi Pembayaran Event ' . $event_name . '';
        $body = get_email_template("notification_payment_email", $subject, ["first_name" => $this->_data['first_name'], "last_name" => $this->_data['last_name'], "email_verification_code" => $url]);
        /* send email */
        if(!_email($email, $subject, $body['html'])) {
            throw new Exception(__("Activation key email could not be sent!"));
        }
    ...
    public function activation_email_resend() {
        ...
        /* send email */
        if(!_email($this->_data['user_email'], $subject, $body['html'])) {
