...
$(function() {
    ...
    $('body').on('click', '.js_publisher', function() {
        ...
        if(is_empty(textarea.val()) && link === undefined && photos === undefined && feeling.val() == "" && location.val() == "" && voice_notes === undefined && poll_options === undefined && video === undefined && audio === undefined && file === undefined) {
            return;
        }
        /* button loading */
    ...
    $('body').on('click', '.js_voice-start', function() {
        ...
        navigator.mediaDevices.getUserMedia({ audio: true, video:false }).then(function(stream) {
            ...
            voice_recording_object = new Recorder(audioContext.createMediaStreamSource(stream));
            /* start the recording process */
            voice_recording_object.record();
    ...
    $('body').on('click', '.js_lightbox-live', function () {
        ...
        /* get live post */
        $.post(api['live/reaction'], {'do': 'join', 'post_id': live_post_id}, function(response) {
            /* check the response */
            if(response.callback) {
                $('body').removeClass('lightbox-open');
                $('.lightbox').remove();
        ...
        /* handle stream-subscribed */
        client.on('stream-subscribed', function (evt) {
            /* hide live status */
            $('#js_live-status').html('').hide();
            /* play live stream */
            var remoteStream = evt.stream;
            remoteStream.play('js_live-video');
            /* handle live counter */
    ...
    $('body').on('click', '.reactions-wrapper', function() {
        if($(window).width() < 970) {
            ...
            if(old_reaction) {
                ...
                _parent.find('.reaction-btn-icon:first').html('<i class="far fa-thumbs-up fa-fw"></i>');
