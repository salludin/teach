...
try {
	...
	switch ($_POST['handle']) {
		...
		case 'offers_category':
			...
			$db->query(sprintf("DELETE FROM offers_categories WHERE category_id = %s", secure($_POST['id'], 'int') )) or _error("SQL_ERROR_THROWEN");
			break;
		...
		case 'jobs_category':
			...
			$db->query(sprintf("DELETE FROM jobs_categories WHERE category_id = %s", secure($_POST['id'], 'int') )) or _error("SQL_ERROR_THROWEN");
			break;
