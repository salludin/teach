...
function get_licence_key($code) {
    $url = 'https://www.sngine.com/licence/v2/verify.php';
...
function email_smtp_test() {
    ...
    /* send email */
    if(!_email($system['system_email'], $subject, $body['html'], true, true)) {
...
function user_access($is_ajax = false) {
    ...
    if($is_ajax) {
        ...
        if(!$user->_logged_in) {
            ...
            if($user->_data['user_group'] == '4') {
                redirect("/pages/{$user->_data['user_name']}");
            }
