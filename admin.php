...
try {
	...
	switch ($_GET['view']) {
		...
		case 'offers':
			...
			switch ($_GET['sub_view']) {
				case '':
					...
					require('includes/class-pager.php');
					$params['selected_page'] = ( (int) $_GET['page'] == 0) ? 1 : $_GET['page'];
					$total = $db->query("SELECT COUNT(*) as count FROM posts INNER JOIN posts_offers ON posts.post_id = posts_offers.post_id AND posts.post_type = 'offer' INNER JOIN pages ON posts.user_id = pages.page_id AND posts.user_type = 'page'") or _error("SQL_ERROR");
					$params['total_items'] = $total->fetch_assoc()['count'];
					$params['items_per_page'] = $system['max_results'];
					$params['url'] = $system['system_url'].'/'.$control_panel['url'].'/offers?page=%s';
					$pager = new Pager($params);
					$limit_query = $pager->getLimitSql();
					$get_rows = $db->query("SELECT posts.*, posts_offers.title, pages.page_name, pages.page_title, pages.page_picture FROM posts INNER JOIN posts_offers ON posts.post_id = posts_offers.post_id AND posts.post_type = 'offer' INNER JOIN pages ON posts.user_id = pages.page_id AND posts.user_type = 'page' ORDER BY posts.post_id ASC ".$limit_query) or _error("SQL_ERROR");
					if($get_rows->num_rows > 0) {
						while($row = $get_rows->fetch_assoc()) {
							$row['page_picture'] = get_picture($row['page_picture'], 'page');
							$rows[] = $row;
				...
				case 'find':
					...
					require('includes/class-pager.php');
					$params['selected_page'] = ( (int) $_GET['page'] == 0) ? 1 : $_GET['page'];
					$total = $db->query(sprintf('SELECT COUNT(*) as count FROM posts INNER JOIN posts_offers ON posts.post_id = posts_offers.post_id AND posts.post_type = "offer" INNER JOIN pages ON posts.user_id = pages.page_id AND posts.user_type = "page" WHERE posts.text LIKE %1$s OR posts_offers.title LIKE %1$s', secure($_GET['query'], 'search') )) or _error("SQL_ERROR");
					$params['total_items'] = $total->fetch_assoc()['count'];
					$params['items_per_page'] = $system['max_results'];
					$params['url'] = $system['system_url'].'/'.$control_panel['url'].'/offers/find?query='.$_GET['query'].'&page=%s';
					$pager = new Pager($params);
					$limit_query = $pager->getLimitSql();
					$get_rows = $db->query(sprintf('SELECT posts.*, posts_offers.title, pages.page_name, pages.page_title, pages.page_picture FROM posts INNER JOIN posts_offers ON posts.post_id = posts_offers.post_id AND posts.post_type = "offer" INNER JOIN pages ON posts.user_id = pages.page_id AND posts.user_type = "page" WHERE posts.text LIKE %1$s OR posts_offers.title LIKE %1$s'.$limit_query, secure($_GET['query'], 'search') )) or _error("SQL_ERROR");
					if($get_rows->num_rows > 0) {
						while($row = $get_rows->fetch_assoc()) {
							$row['page_picture'] = get_picture($row['page_picture'], 'page');
							$rows[] = $row;
				...
				case 'categories':
					...
					// get data
					$rows = $user->get_offers_categories(false);
				...
				case 'edit_category':
					...
					// get data
					$get_data = $db->query(sprintf("SELECT * FROM offers_categories WHERE category_id = %s", secure($_GET['id'], 'int') )) or _error("SQL_ERROR");
					if($get_data->num_rows == 0) {
						_error(404);
					}
					$data = $get_data->fetch_assoc();
		...
		case 'jobs':
			...
			switch ($_GET['sub_view']) {
				case '':
					...
					require('includes/class-pager.php');
					$params['selected_page'] = ( (int) $_GET['page'] == 0) ? 1 : $_GET['page'];
					$total = $db->query("SELECT COUNT(*) as count FROM posts INNER JOIN posts_jobs ON posts.post_id = posts_jobs.post_id AND posts.post_type = 'job' INNER JOIN pages ON posts.user_id = pages.page_id AND posts.user_type = 'page'") or _error("SQL_ERROR");
					$params['total_items'] = $total->fetch_assoc()['count'];
					$params['items_per_page'] = $system['max_results'];
					$params['url'] = $system['system_url'].'/'.$control_panel['url'].'/jobs?page=%s';
					$pager = new Pager($params);
					$limit_query = $pager->getLimitSql();
					$get_rows = $db->query("SELECT posts.*, posts_jobs.title, pages.page_name, pages.page_title, pages.page_picture FROM posts INNER JOIN posts_jobs ON posts.post_id = posts_jobs.post_id AND posts.post_type = 'job' INNER JOIN pages ON posts.user_id = pages.page_id AND posts.user_type = 'page' ORDER BY posts.post_id ASC ".$limit_query) or _error("SQL_ERROR");
					if($get_rows->num_rows > 0) {
						while($row = $get_rows->fetch_assoc()) {
							$row['page_picture'] = get_picture($row['page_picture'], 'page');
							$rows[] = $row;
				...
				case 'find':
					...
					require('includes/class-pager.php');
					$params['selected_page'] = ( (int) $_GET['page'] == 0) ? 1 : $_GET['page'];
					$total = $db->query(sprintf('SELECT COUNT(*) as count FROM posts INNER JOIN posts_jobs ON posts.post_id = posts_jobs.post_id AND posts.post_type = "job" INNER JOIN pages ON posts.user_id = pages.page_id AND posts.user_type = "page" WHERE posts.text LIKE %1$s OR posts_jobs.title LIKE %1$s', secure($_GET['query'], 'search') )) or _error("SQL_ERROR");
					$params['total_items'] = $total->fetch_assoc()['count'];
					$params['items_per_page'] = $system['max_results'];
					$params['url'] = $system['system_url'].'/'.$control_panel['url'].'/jobs/find?query='.$_GET['query'].'&page=%s';
					$pager = new Pager($params);
					$limit_query = $pager->getLimitSql();
					$get_rows = $db->query(sprintf('SELECT posts.*, posts_jobs.title, pages.page_name, pages.page_title, pages.page_picture FROM posts INNER JOIN posts_jobs ON posts.post_id = posts_jobs.post_id AND posts.post_type = "job" INNER JOIN pages ON posts.user_id = pages.page_id AND posts.user_type = "page" WHERE posts.text LIKE %1$s OR posts_jobs.title LIKE %1$s'.$limit_query, secure($_GET['query'], 'search') )) or _error("SQL_ERROR");
					if($get_rows->num_rows > 0) {
						while($row = $get_rows->fetch_assoc()) {
							$row['page_picture'] = get_picture($row['page_picture'], 'page');
							$rows[] = $row;
				...
				case 'categories':
					...
					// get data
					$rows = $user->get_jobs_categories(false);
				...
				case 'edit_category':
					...
					// get data
					$get_data = $db->query(sprintf("SELECT * FROM jobs_categories WHERE category_id = %s", secure($_GET['id'], 'int') )) or _error("SQL_ERROR");
					if($get_data->num_rows == 0) {
						_error(404);
					}
					$data = $get_data->fetch_assoc();
		...
		case 'tools':
			...
			switch ($_GET['sub_view']) {
				...
				case 'garbage-collector':
			    	...
			    	$get_users_not_logged_3_months = $db->query("SELECT COUNT(*) as count FROM users WHERE user_last_seen < NOW() - INTERVAL 3 MONTH") or _error("SQL_ERROR");
			    	$insights['users_not_logged_3_months'] = $get_users_not_logged_3_months->fetch_assoc()['count'];
			    	/* users not logged in 1 month */
			    	$get_users_not_logged_6_months = $db->query("SELECT COUNT(*) as count FROM users WHERE user_last_seen < NOW() - INTERVAL 6 MONTH") or _error("SQL_ERROR");
			    	$insights['users_not_logged_6_month'] = $get_users_not_logged_6_months->fetch_assoc()['count'];
			    	/* users not logged in 1 month */
			    	$get_users_not_logged_9_months = $db->query("SELECT COUNT(*) as count FROM users WHERE user_last_seen < NOW() - INTERVAL 9 MONTH") or _error("SQL_ERROR");
			    	$insights['users_not_logged_9_month'] = $get_users_not_logged_9_months->fetch_assoc()['count'];
		...
		case 'newsletter':
			...
			// prepare counts
			/* all users */
		    $get_users_all = $db->query("SELECT COUNT(*) as count FROM users WHERE user_activated = '1'") or _error("SQL_ERROR");
	    	$insights['users_all'] = $get_users_all->fetch_assoc()['count'];
			/* users activated */
		    $get_users_activated = $db->query("SELECT COUNT(*) as count FROM users WHERE user_activated = '1'") or _error("SQL_ERROR");
