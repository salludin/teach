...
try {
	...
	switch ($_GET['view']) {
		case '':
			...
			// page header
			page_header($system['system_title'].' - '.__("Ads Manager"));
		...
		case 'views':
			...
			// page header
			page_header(__("Views"));

			// get campaigns
			$campaigns = $user->get_campaign_analyst($_GET['campaign_id']);
			/* assign variables */
			$smarty->assign('campaigns', $campaigns);
			break;
		...
		case 'wallet':
			...
			// page header
			page_header($system['system_title'].' - '.__("Wallet"));
